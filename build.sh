...
build.sh - compile godag source

the 'install' target will try to move the binary
generated by the compile to the directory \$HOME/bin.
if that directory is not present it will not be
created, and the binary 'gd' needs to be moved to a
directory in \$PATH in order to be found.

the 'cproot' target constructs a copy of the *.go files
under \$GOROOT/src/pkg, but it filters out 'pure go'
source, and renames files and moves files according to
their natural namespace.
